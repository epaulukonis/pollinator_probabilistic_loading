...
final_Illinois_by_crop<-final_Illinois_by_crop[final_Illinois_by_crop$percent>4,] #get crops that make up 95% of crop area
...
# names(list_of_final_data_by_countyI_sf)<-cntynames
# for(i in 1:length(list_of_final_data_by_countyI)){
#   write.csv(list_of_final_data_by_countyI[i], paste0(root_figures, "/", names(list_of_final_data_by_countyI)[i], "_finaldf_sf.csv"))
# }


cntynames<-c('CHAMPAIGN',"DU PAGE","MCHENRY")
list_of_plotsI<-list()
list_of_dataI_sf<-list()
for(c in 1:length(list_of_final_data_by_countyI_sf)){
  layer_by_year_crops<-list_of_final_data_by_countyI_sf[[c]]
  extracted_cdl_dataI$CDLacres<-((extracted_cdl_dataI$count)*900)*0.000247105
  colnames(extracted_cdl_dataI)[1]<-'Class'
  layer_by_year_crops<-left_join(layer_by_year_crops, extracted_cdl_dataI, by=c("County","year","Class"))
...
#Plot showing the ratio of NASS acres to field acres across years, by threshold, crop
final_Illinois_by_cropsf<-do.call(rbind, list_of_final_data_by_countyI_sf)
final_Illinois_by_cropsf$Label<-paste0(tools::toTitleCase(tolower(final_Illinois_by_cropsf$County))," County, Illinois")
final_Illinois_by_cropsf$Label<-ifelse(final_Illinois_by_cropsf$Label == "Du Page County, Illinois","DuPage County, Illinois", final_Illinois_by_cropsf$Label)
final_Illinois_by_cropsf$Label<-ifelse(final_Illinois_by_cropsf$Label == "Mchenry County, Illinois","McHenry County, Illinois", final_Illinois_by_cropsf$Label)
final_Illinois_by_cropsf$County <- factor(final_Illinois_by_cropsf$County , levels = c("CHAMPAIGN", "MCHENRY", "DU PAGE"))
final_Illinois_by_cropsf$ratio<-final_Illinois_by_cropsf$fieldacres/final_Illinois_by_cropsf$NASSacres
final_Illinois_by_cropsf <- final_Illinois_by_cropsf %>%
  group_by(County, year, threshold) %>% mutate(percent = (NASSacres/sum(NASSacres))*100) #get percentage of each crop
final_Illinois_by_cropsf<-final_Illinois_by_cropsf[final_Illinois_by_cropsf$percent>4,] #get crops that make up 95% of crop area
final_Illinois_by_cropsf<-final_Illinois_by_cropsf[final_Illinois_by_cropsf$threshold ==1, ]


# #### Michigan acreages HP----
cntynames<-c("HURON", "OCEANA", "VAN BUREN")
...
list_of_final_data_by_countyM_sf<-list()
for(n in 1:length(acreages_by_countyM)){
  county<-acreages_by_countyM[[n]]
  county_name<-names(acreages_by_countyM[n])
  names(county)<-years
...
# for(i in 1:length(list_of_final_data_by_countyM)){
#   write.csv(list_of_final_data_by_countyM[i], paste0(root_figures, "/", names(list_of_final_data_by_countyM)[i], "_finaldf_sf.csv"))
# }
...
list_of_plotsM<-list()
list_of_dataM_sf<-list()
for(c in 1:length(list_of_final_data_by_countyM_sf)){
  layer_by_year_crops<-list_of_final_data_by_countyM_sf[[c]]
  extracted_cdl_dataM$CDLacres<-((extracted_cdl_dataM$count)*900)*0.000247105
  colnames(extracted_cdl_dataM)[1]<-'Class'
  layer_by_year_crops<-left_join(layer_by_year_crops, extracted_cdl_dataM, by=c("County","year","Class"))
...
#Plot showing the ratio of NASS acres to field acres across years, by crop at threshold 1
final_Michigan_by_cropsf<-do.call(rbind, list_of_final_data_by_countyM_sf)
final_Michigan_by_cropsf$Label<-paste0(tools::toTitleCase(tolower(final_Michigan_by_cropsf$County))," County, Michigan")
final_Michigan_by_cropsf$County <- factor(final_Michigan_by_cropsf$County , levels = c("HURON", "OCEANA", "VAN BUREN"))
final_Michigan_by_cropsf$ratio<-final_Michigan_by_cropsf$fieldacres/final_Michigan_by_cropsf$NASSacres
final_Michigan_by_cropsf <- final_Michigan_by_cropsf %>%
  group_by(County, year, threshold) %>% mutate(percent = (NASSacres/sum(NASSacres))*100) #get percentage of each crop
final_Michigan_by_cropsf<-final_Michigan_by_cropsf[final_Michigan_by_cropsf$percent>4,] #get crops that make up ~95% of crop area
final_Michigan_by_cropsf<-final_Michigan_by_cropsf[final_Michigan_by_cropsf$threshold ==1, ]


### Wisconsin Acreages HP----
...
list_of_final_data_by_countyW_sf<-list()
for(n in 1:length(acreages_by_countyW)){
  county<-acreages_by_countyW[[n]]
  county_name<-names(acreages_by_countyW[n])
  names(county)<-years
...
# for(i in 1:length(list_of_final_data_by_countyW)){
#   write.csv(list_of_final_data_by_countyW[i], paste0(root_figures, "/", names(list_of_final_data_by_countyW)[i], "_finaldf_sf.csv"))
# }
...
#Plot showing the ratio of NASS acres to field acres across years, by crop at threshold 1
final_Wisconsin_by_cropsf<-do.call(rbind, list_of_final_data_by_countyW_sf)
final_Wisconsin_by_cropsf$Label<-paste0(tools::toTitleCase(tolower(final_Wisconsin_by_cropsf$County))," County, Wisconsin")
final_Wisconsin_by_cropsf$County <- factor(final_Wisconsin_by_cropsf$County , levels = c("ROCK", "WAUSHARA", "LANGLADE"))
final_Wisconsin_by_cropsf$ratio<-final_Wisconsin_by_cropsf$fieldacres/final_Wisconsin_by_cropsf$NASSacres
final_Wisconsin_by_cropsf <- final_Wisconsin_by_cropsf %>%
  group_by(County, year, threshold) %>% mutate(percent = (NASSacres/sum(NASSacres))*100) #get percentage of each crop
final_Wisconsin_by_cropsf<-final_Wisconsin_by_cropsf[final_Wisconsin_by_cropsf$percent>4,] #get crops that make up ~95% of crop area
final_Wisconsin_by_cropsf<-final_Wisconsin_by_cropsf[final_Wisconsin_by_cropsf$threshold ==1, ]
