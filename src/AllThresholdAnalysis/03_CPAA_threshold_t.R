...
if(file.exists(thresh_mi_filename)){
  ...
  # thresh_list_mi_f[[1]]<-thresh_list_mi[1:3]
  # thresh_list_mi_f[[2]]<-thresh_list_mi[4:6]
  # thresh_list_mi_f[[3]]<-thresh_list_mi[7:8]
  ...
  sub<-study[study$NAME %in% sub_group,]
  sub<-spTransform(sub, crs(cdl_data_mi_rec[[1]])) #reproject
  ...
  #writeRaster(chosen_count[[3]][[1]], paste0(root_data_out,"/all_tif/MICHIGAN/CPAA/Evaluation/cdl_raw_examination"), format="GTiff", overwrite = TRUE)
  ...
  writeRaster(county_list[[3]][[1]], paste0(root_data_out,"/all_tif/MICHIGAN/CPAA/Evaluation/cdl_reclass_examination"), format="GTiff", overwrite = TRUE)
...
if(file.exists(thresh_wi_filename)){
  ...
  sub<-study[study$NAME %in% sub_group,]
  sub<-spTransform(sub, crs(cdl_data_wi_rec[[1]])) #reproject
