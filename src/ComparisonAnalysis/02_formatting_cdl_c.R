...
### Michigan-Specific 
cdl_rec_filename<-paste0(cdl_mi_dir_adj, "/CDL_2021_17.tif")
if(file.exists(cdl_rec_filename)){
  
  print(list.files(path=cdl_mi_dir_adj, pattern='.tif$', all.files=TRUE, full.names=FALSE))
  cdl_data_mi_rec <- file.path(cdl_mi_dir_adj, list.files(path=cdl_mi_dir_adj, pattern='.tif$', all.files=TRUE, full.names=FALSE))
...
cdl_fin_co<-lapply(cdl_data, mask_crop)
