...
names(county_rec_list)<-c("Champaign","DuPage","McHenry") ##double check here
...
for (county in 1:length(sub_by_county_list)){
  ...
  for (year in 1:length(sub_list)){
    ...
    #focal window: 10 acres (why? bc that's the minimum average field size from the LACIE paper)
    ...
    f<-freq(rc)
    f<-as.data.frame(f)
    excludeID <- f$value[which(f$count <= 3)] #remove pixel clumps (visual inspection) #3 = 2 acres
    formaskSieve <- rc
...
for(cpaal in 1:length(cpaa_list_ill_f)){
  ...
  for(i in 1:length(output)){
    ...
    setwd(paste0(root_data_out, "/all_tif/ILLINOIS/SUB/smallerfields"))
