...
#here, we'll extract the CDL data to the field layer we're interested in. For the purposes of the paper
#I am really only interested in 2021 data for Champaign County, Illinois. 
...
#county_level_vectors<-readRDS(paste0(root_data_out,"/county_level_field_vectors.RData"))
...
# champ1<-cbind(champ1,county_level_vectors[[1]])
# colnames(champ1)
...
for(f in 1:length(field_list_ill_f[[1]])){
champ1<-st_as_sf(fields)# get field data 
cdl_extract<-field_extractions_cdl[[f]] # get cdl data
plot(fields)

champ1<-cbind(champ1,cdl_extract)
colnames(champ1)
colnames(champ1)[3]<-"crop"
colnames(champ1)[4]<-"area"
...
st_write(fw_fill, root_data_out, layer=paste0("final_champ1_", years[f], ".shp"), driver = "ESRI Shapefile")
