...
for(layer in 1:length(three_county_list)){
  ...
  list_freq2<-list()
    for(f in 1:length(county)){
      ...
      df_crop_areas[,2:12]<-(df_crop_areas[,2:12]*900 ) * 0.000247105 #get area in acres
...
for(i in nrow(n_pixels):2){
  n_pixels[i,3]<-(sum(n_pixels[i:12,2])*900)*0.000247105
...
##for low: min bin is 1
##for med: min bin is 6
##for high: min bin is 11
