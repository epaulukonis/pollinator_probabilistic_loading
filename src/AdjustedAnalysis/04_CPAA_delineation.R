...
county_fw_sets<-list()
  ...
  df_n<-thresh_layers[,c(1:2,8)]
    ...
    f<-freq(rc)
    f<-as.data.frame(f)
    excludeID <- f$value[which(f$count <= 9)] #remove pixel clump of 9 or fewer
    formaskSieve <- rc
...
area_thresh <- units::set_units(8100, m^2) # 9 pixels 
expand_shrink_clean<-function(x){
  expand<-gBuffer(x, width=60, byid=T)
  shrink<-gBuffer(expand, width=-60, byid=T)
...
# 
# 
# writeOGR(output_of_cleaning[[1]], cdl_dir,  "/low_clean", driver = "ESRI Shapefile")
# writeOGR(output_of_cleaning[[2]], cdl_dir,  "/med_clean", driver = "ESRI Shapefile")
# writeOGR(output_of_cleaning[[3]], cdl_dir,  "/high_clean", driver = "ESRI Shapefile")
